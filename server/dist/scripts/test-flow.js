...
const apiCall = async (method, path, token = null, body = null) => {
    const headers = {
...
const runVerificationTests = async () => {
    try {
        ...
        for (const u of users) {
            await user_model_1.User.create(u);
        }
        ...
        if (res.status !== 400 || !res.data.errors.some((e) => e.includes('salary'))) {
        ...
        if (res.status !== 400 || !res.data.errors.some((e) => e.includes('Age'))) {
        ...
        if (res.status !== 400 || !res.data.errors.some((e) => e.includes('Employment'))) {
        ...
        if (res.status !== 400 || !res.data.errors.some((e) => e.includes('PAN'))) {
