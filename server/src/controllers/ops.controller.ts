 ...
 */
export const getSalesData = async (_req: Request, res: Response): Promise<void> => {
  try {
    // 1. Get borrowerId lists from existing Loan applications
    const appliedBorrowerIds = await Loan.distinct('borrowerId');
 ...
 */
export const getSanctionData = async (_req: Request, res: Response): Promise<void> => {
  try {
    const pendingLoans = await Loan.find({ status: 'Pending' })
      .populate('borrowerId', 'name email')
      .sort({ createdAt: 1 });

    const borrowerProfiles = await BorrowerProfile.find({
      userId: { $in: pendingLoans.map((l) => l.borrowerId._id) },
    });

    const loansWithProfiles = pendingLoans.map((loan) => {
      const profile = borrowerProfiles.find(
        (p) => p.userId.toString() === loan.borrowerId._id.toString()
      );
      return {
        loan,
        profile: profile || null,
 ...
 * @access  Private (Sanction or Admin)
 */
export const decideSanction = async (req: Request, res: Response): Promise<void> => {
  try {
    if (!req.user) {
    ...
    let toStatus: 'Sanctioned' | 'Rejected';
    let comments = '';
 ...
 */
export const getDisbursementData = async (_req: Request, res: Response): Promise<void> => {
  try {
    const sanctionedLoans = await Loan.find({ status: 'Sanctioned' })
      .populate('borrowerId', 'name email')
      .sort({ updatedAt: 1 });
 ...
 */
export const getCollectionData = async (_req: Request, res: Response): Promise<void> => {
  try {
...
export const recordPayment = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const existingUtr = await Payment.findOne({ utr: utr.trim() });
    if (existingUtr) {
      res.status(400).json({
        success: false,
...
export const getAdminOverview = async (_req: Request, res: Response): Promise<void> => {
  try {
    ...
    const rejectedCount = await Loan.countDocuments({ status: 'Rejected' });

    // Aggregate total disbursements and interest
    const loanRepaymentAgg = await Loan.aggregate([
      {
        $group: {
          _id: null,
